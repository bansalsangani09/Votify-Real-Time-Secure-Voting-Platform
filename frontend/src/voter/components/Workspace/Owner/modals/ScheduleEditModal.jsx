...
const ScheduleEditModal = ({ isOpen, onClose, electionData, onUpdate }) => {
    ...
    useEffect(() => {
        if (electionData) {
            ...
            // Correctly format local date as YYYY-MM-DD
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            // Combine date and time into ISO strings to preserve timezone
            const combinedStart = start.toISOString();
            const combinedEnd = end.toISOString();
            const combinedResult = formData.resultTime ? new Date(formData.resultTime).toISOString() : null;

            const res = await api.patch(`/elections/${electionData._id}`, {
                ...formData,
                startDate: combinedStart,
                endDate: combinedEnd,
                resultTime: combinedResult
            });
