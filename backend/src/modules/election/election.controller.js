...
import { parseDateTime } from '../../utils/election.util.js';
...
export const createElection = async (req, res, next) => {
    ...
    try {
        ...
        const fullStart = parseDateTime(startDate, startTime);
        const fullEnd = parseDateTime(endDate, endTime);
        const now = new Date();
...
export const updateElection = async (req, res, next) => {
    try {
        ...
        if (updates.startDate) {
            updates.startDate = parseDateTime(updates.startDate, updates.startTime || election.startTime);
        }
        if (updates.endDate) {
            updates.endDate = parseDateTime(updates.endDate, updates.endTime || election.endTime);
        }
        if (updates.resultTime) {
            updates.resultTime = parseDateTime(updates.resultTime);
        }
