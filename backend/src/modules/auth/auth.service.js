...
export const login = async (email, password, ipAddress, userAgent) => {
    ...
    if (!user) {
        console.log(`\x1b[33m[Login-Auth]\x1b[0m User not found: ${normalizedEmail}`);
        return { success: false, message: 'Invalid email or password' };
    }

    const isMatch = await bcrypt.compare(password, user.passwordHash);
    if (!isMatch) {
        console.log(`\x1b[33m[Login-Auth]\x1b[0m Password mismatch for: ${normalizedEmail}`);
        return { success: false, message: 'Invalid email or password' };
    }

    // Role-based 2FA (Magic Link) Logic
    if (user.role === 'admin') {
        const settings = await Settings.findOne() || { twoFactorAuthentication: false };
