...
import { updateSettingsSchema, transferOwnershipSchema } from '../modules/admin/admin.validation.js';
import { 
...
router.post('/voters', protect, admin, validate(createVoterSchema), userController.createVoter);
router.get('/voters/:id/profile', protect, admin, userController.getVoterProfile);
router.patch('/voters/:id/status', protect, admin, validate(updateVoterStatusSchema), userController.updateVoterStatus);
router.post('/voters/:id/remove-from-election', protect, admin, validate(removeFromElectionSchema), userController.removeFromElection);

router.get('/owners', protect, admin, userController.getOwners);
router.patch('/owners/:id/status', protect, admin, validate(updateVoterStatusSchema), userController.updateOwnerStatus);
...
router.post('/settings', protect, admin, validate(updateSettingsSchema), adminController.updateSettings);
router.post('/transfer-ownership', protect, admin, validate(transferOwnershipSchema), adminController.transferOwnership);
